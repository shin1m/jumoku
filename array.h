...
class t_array : public t_tree<T_traits, A_branch>
{
	...
	struct t_leaf : t_link
	{
		...
		t_delta f_place(size_t a_i)
		{
			auto p = f_values() + a_i;
			auto delta = T_traits::f_index(0, *p);
		...
		void f_insert(size_t a_i, T&& a_value)
		{
			...
			++this->v_size;
			f_add(p + a_i + 1, p + this->v_size, f_place(a_i));
		}
		template<typename T>
		void f_insert(t_leaf* a_p, size_t a_i, T&& a_value)
		{
			...
			f_add(p, p + a_i, delta);
			f_add(p + a_i + 1, p + A_leaf, delta + f_place(a_i));
		}
		template<typename T>
		void f_insert(size_t a_i, T&& a_value, t_leaf* a_p)
		{
			...
			f_add(p + a_i + 1, p + A_leaf, f_place(a_i));
		}
		template<typename T>
	...
	template<typename T>
	t_at f_insert_leaf(t_via* a_head, t_via* a_tail, const t_at& a_at, T a_first, t_index a_n);
...
public:
	...
	{
		this->v_size -= T_traits::f_index(1, *a_i);
	...
	t_mutable_iterator f_erase(t_constant_iterator a_first, t_constant_iterator a_last)
	{
		...
		if (i + n > a_first.v_leaf->v_size) {
			...
			at = f_at(index, path, T_traits::f_default);
...
template<typename T_value, size_t A_leaf, size_t A_branch, typename T_traits>
template<typename T>
typename t_array<T_value, A_leaf, A_branch, T_traits>::t_at t_array<T_value, A_leaf, A_branch, T_traits>::f_insert_leaf(t_via* a_head, t_via* a_tail, const t_at& a_at, T a_first, t_index a_n)
{
	// |= N  =| |= N =| |= N  =|
	// |=n0=|.| |=n=|.| |=n1=|.|
	//             ^i|m|
...
typename t_array<T_value, A_leaf, A_branch, T_traits>::t_at t_array<T_value, A_leaf, A_branch, T_traits>::f_erase_leaf(t_via* a_head, t_via* a_tail, const t_at& a_at, size_t a_n)
{
	...
	if (a_head == a_tail) {
		...
		this->f_adjust(a_head, a_tail, -p->f_erase(i, a_n));
