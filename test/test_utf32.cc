...
{
	{
		...
		auto i = jumoku::f_utf32(utf8.f_begin());
		auto j = jumoku::f_utf32(utf8.f_end());
		assert(i != j);
		assert(*i == L'$');
		assert(++i != j);
		assert(*i == L'\u00a3');
		...
		assert(*i == L'\u20ac');
		assert(++i != j);
		assert(*i == L'\U00010348');
		assert(++i == j);
		i = jumoku::f_utf32(utf8.f_begin());
		assert(j != i);
		assert(*--j == L'\U00010348');
		assert(j != i);
		assert(*--j == L'\u20ac');
		assert(j != i);
		assert(*--j == L'\u0939');
	...
	}
	{
		jumoku::t_utf8<5, 5> utf8;
		{
			...
			std::copy_n(L"$\u00a3\u0939\u20ac\U00010348", 5, jumoku::f_utf32_output(i));
		}
