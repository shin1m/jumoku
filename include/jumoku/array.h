...
typename t_array<T_value, A_leaf, A_branch, T_traits>::t_at t_array<T_value, A_leaf, A_branch, T_traits>::f_insert_leaf(t_via* a_head, t_via* a_tail, const t_at& a_at, auto&& a_value)
{
	...
	if (a_head != a_tail) {
		auto [q, j] = a_tail[-1];
		if (j > 0) if (auto r = p->v_previous; r->v_size < A_leaf) {
			assert(r->v_size > 0);
			this->f_adjust(a_head, a_tail, delta);
			if (i > 0) {
		...
		if (j < q->v_size) if (auto r = p->v_next; r->v_size < A_leaf) {
...
typename t_array<T_value, A_leaf, A_branch, T_traits>::t_at t_array<T_value, A_leaf, A_branch, T_traits>::f_insert_leaf(t_via* a_head, t_via* a_tail, const t_at& a_at, auto a_first, size_t a_n)
{
	...
	if (l.v_node == p && ii + a_n + ni > A_leaf * 2) {
		...
		if (ii <= ni) {
			...
			if (n > 0) {
				f_move(v + i, v + p->v_size, r->f_values() + i);
				p->v_size = A_leaf;
				std::uninitialized_copy_n(a_first, n, v + ii);
				auto delta = p->f_place(ii, n);
				a_tail = this->f_insert_branch(a_head, a_tail, T_traits::f_index(A_leaf, v[A_leaf - 1]), delta, r, true, true);
				this->v_size += delta;
			} else {
				a_tail = this->f_insert_branch(a_head, a_tail, T_traits::f_index(A_leaf, v[A_leaf - 1]), {}, r, true, true);
			}
			p = r;
			v = r->f_values();
		}
...
typename t_array<T_value, A_leaf, A_branch, T_traits>::t_at t_array<T_value, A_leaf, A_branch, T_traits>::f_erase_leaf(t_via* a_head, t_via* a_tail, const t_at& a_at, size_t a_n)
{
	...
	if (j > 0) {
		if (auto r = p->v_previous; r->v_size - n >= A_leaf / 2) {
			...
			q->v_indices[j - 1] -= T_traits::f_index(n, p->f_values()[n - 1]);
			this->f_adjust(a_head, a_tail, -delta);
			this->v_size -= delta;
			i += n;
	...
	if (auto r = p->v_next; r->v_size - n >= A_leaf / 2) {
		auto size = T_traits::f_index(n, r->f_values()[n - 1]);
